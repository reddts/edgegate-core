...
func GetOverridableCoreOptions(overrides map[string][]string) *CoreOptions {
	overrideCore := CoreOptions{}

	// Convert flat overrides to nested structure
	nestedOverrides := convertFlatToNested(overrides)

	// Use reflection to iterate over the fields of CoreOptions
	v := reflect.ValueOf(&overrideCore).Elem()
	t := reflect.TypeOf(overrideCore)
