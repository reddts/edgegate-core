...
import (
	...
	"github.com/reddts/edgegate-core/v2/hcommon"
	"github.com/reddts/edgegate-core/v2/hutils"
...
func (s *CoreRPCServer) Setup(ctx context.Context, req *SetupRequest) (*hcommon.Response, error) {
...
func Setup(params *SetupRequest, platformInterface libbox.PlatformInterface) error {
	...
	val, err := settings.Get("CoreSettingsJson")
	if val == nil || err != nil {
		// if params.Mode == SetupMode_GRPC_BACKGROUND_INSECURE {
		_, err := ChangeCoreSettings(&ChangeCoreSettingsRequest{CoreSettingsJson: ""})
		if err != nil {
			Log(LogLevel_DEBUG, LogType_CORE, E.Cause(err, "ChangeCoreSettings").Error())
		}
	} else {
		// settings := db.GetTable[hcommon.AppSettings]()
		_, err := ChangeCoreSettings(&ChangeCoreSettingsRequest{CoreSettingsJson: val.Value.(string)})
		if err != nil {
			Log(LogLevel_DEBUG, LogType_CORE, E.Cause(err, "ChangeCoreSettings").Error())
		}
