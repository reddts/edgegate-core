...
type grpcTunnelRunner struct {
	tunnelService *TunnelService
}

var port int = 18020

func (m *grpcTunnelRunner) StartTunnelGrpcServer(listenAddressG string) (*grpc.Server, error) {
...
func (m *grpcTunnelRunner) Start(s service.Service) error {
	_, err := m.StartTunnelGrpcServer(fmt.Sprintf("127.0.0.1:%d", port))
	return err
}

func (m *grpcTunnelRunner) Stop(s service.Service) error {
...
func StartTunnelService(goArg string) (int, string) {
	svcConfig := &service.Config{
		Name:        "EdgegateTunnelService",
		DisplayName: "Edgegate Tunnel Service",
